...
function itemMoreInfo(product) {
    ...
    leftDiv.setAttribute("class", "container border border-1 border-dark d-flex flex-column justify-content-around");
    leftDiv.setAttribute("style", "width:48%; box-shadow:10px 10px 10px grey;");
    itemContainer.appendChild(leftDiv);

    let leftDivMainImage = document.createElement("div");
    leftDivMainImage.setAttribute("style", "width:100%;height:70%; box-sizing:border-box;")
    leftDivMainImage.setAttribute("class", "col border border-1 border-danger");
    leftDiv.appendChild(leftDivMainImage);

    let mainImage = document.createElement("img");
    mainImage.setAttribute("style", "width:100%;height:380px;")
    mainImage.src = product.thumbnail;
    leftDivMainImage.appendChild(mainImage);

    let leftImagesDiv = document.createElement("div");
    leftImagesDiv.setAttribute("style", "width:100%;height:20%;")
    leftImagesDiv.setAttribute("class", "col border border-1 border-warning mt-3 d-flex align-items-center justify-content-around")
    leftDiv.appendChild(leftImagesDiv);

    product.images.forEach(smallImage => {
        let minImageDiv = document.createElement("div");
        minImageDiv.setAttribute("style", "width:100px;height:100px;");
        minImageDiv.setAttribute("class", "border border-1 border-dark")
        leftImagesDiv.appendChild(minImageDiv);

        let shortImage = document.createElement("img");
        shortImage.setAttribute("style", "width:100px;height:100px;")
        shortImage.src = smallImage;
        minImageDiv.appendChild(shortImage);

        /// Image swap funtion 
        shortImage.addEventListener('click', () => {
            let temp = shortImage.src;
            // shortImage.src=mainImage.src;
            mainImage.src = temp;
            minImageDiv.setAttribute("class", "border border-1 border-warning")
        })
    ...
    rightDiv.setAttribute("class", "border border-1 border-warning p-2");
    rightDiv.setAttribute("style", "width:49%;height:92%;box-shadow:10px 10px 10px grey;")
    itemContainer.appendChild(rightDiv);

    let titleAndDiv = document.createElement("div");
    titleAndDiv.setAttribute("class", "border border-success d-flex justify-content-between p-2");
    titleAndDiv.setAttribute("style", "width:100%;heigth:40px;")
    rightDiv.appendChild(titleAndDiv);

    let h6 = document.createElement("h5");
    h6.innerText = product.title;
    titleAndDiv.appendChild(h6);

    let prevPrice = document.createElement("h5");
    prevPrice.innerText = product.price;
    prevPrice.setAttribute("style", "text-decoration:line-through;");
    prevPrice.setAttribute("class", "text-danger")
    titleAndDiv.appendChild(prevPrice);
    ...
    let warranty = document.createElement("div");
    rightDiv.appendChild(warranty);

    let warrantyInfo = document.createElement("p");
    warrantyInfo.innerHTML = `<b>Warranty Info : </b>${product.warrantyInformation} `;
    rightDiv.appendChild(warrantyInfo);

    let shipingInfo = document.createElement("p");
    shipingInfo.innerHTML = `<b>Shiping Info : </b>${product.shippingInformation} `;
    rightDiv.appendChild(shipingInfo);

    let returnInfo = document.createElement("p");
    returnInfo.innerHTML = `<b>Return Info : </b>${product.returnPolicy} `;
    rightDiv.appendChild(returnInfo);

    let discountInfo = document.createElement("p");
    let priceAfterDiscount = Math.floor(product.price - (product.price * (product.discountPercentage / 100)))
    discountInfo.innerHTML = `<b>After Discount : </b>${priceAfterDiscount}rs.<b style="color:red;"> ${product.discountPercentage}% off</b>`;
    rightDiv.appendChild(discountInfo);
...
function cartContainer() {
    ...
    cartdivContainer.appendChild(cartRow);

    let productList = getData();
    for (let product of productList) {
        let cartDiv = document.createElement("div");
        cartDiv.setAttribute("style", "height:500px;box-shadow:10px 10px 10px grey;");
        cartDiv.setAttribute("class", "col-md-3 d-flex flex-column align-items-center mt-4")
        cartRow.appendChild(cartDiv);
        ...
        productTitle.setAttribute("class", "text-primary text-center");
        cartDiv.appendChild(productTitle);

        let productDiscription = document.createElement("p");
        productDiscription.innerText = `${product.description.substring(0, 30)}....`
        cartDiv.appendChild(productDiscription);

        let seeMore = document.createElement("button");
        seeMore.innerText = "See More";
        seeMore.setAttribute("style", "position:absolute; bottom :10px; width:200px;height:40px;");
        seeMore.setAttribute("class", "bg-warning text-link border border-1 rounded");
        seeMore.addEventListener("click", (eve) => {
            main.removeChild(cartdivContainer);
            // cartdivContainer.innerText = " ";
            itemMoreInfo(product);
        });


        cartDiv.appendChild(seeMore);
    }
...
function signIn() {
    ...
    usernameError.setAttribute("class", "text-danger");
    // usernameError.innerText= "hi i'm error message"
    usernameDiv.appendChild(usernameError);

    let usernameReturn;
    let userNameValue = "";
    usernameInput.addEventListener('input', (event) => {
        userNameValue = event.target.value;
        usernameReturn = usernameValidation(event.target.value);
        if (usernameReturn == 0) {
            console.log("error 1");
    ...
    let usernamePassDiv = document.createElement("div");
    usernamePassDiv.setAttribute("class", "form-group");
    ...
    usernamePassDiv.appendChild(passwordInput);


    let passwordError = document.createElement("small");
    passwordError.setAttribute("class", "text-danger");

    usernamePassDiv.appendChild(passwordError);
    let passwordValidReturn;
    let userPassValue = "";
    passwordInput.addEventListener("keydown", (event) => {
        passwordValidReturn = userPassValidation(event.target.value);
        userPassValue = event.target.value;
        if (passwordValidReturn == 0) {
            passwordError.innerText = " password is required";
        }
        else if (passwordValidReturn == 1) {
            passwordError.innerText = " password should contain more than 8 character or numbers";
        }
        else {
    ...
    signInButton.setAttribute("class", "form-control btn-success text-white");
    signInButton.innerText = "signIn";
    form.appendChild(signInButton);

    let signInErrorMessage = document.createElement("small");
    signInErrorMessage.setAttribute("class", "text-danger");
    ...
    form.addEventListener('submit', (event) => {
        // signInError = signInValidation();
        event.preventDefault();
        let signInUsernameError = usernameValidation(userNameValue);
        let signInUserPassError = userPassValidation(userPassValue);
        if (!signInUsernameError || !signInUserPassError) {
            signInErrorMessage.innerText = "fill above fields";
            usernameError.innerText = "username is required";
            passwordError.innerText = "password is required";
            return;
...
function loadElement() {
    ...
    logoLabel.innerHTML = '<b class="text-white"><span class="text-danger">E-shop</span><h6 class="text-white text-center">Shop on a single click</h6></b>'
    logoDiv.appendChild(logoLabel);

    let searchDiv = document.createElement("div");
    searchDiv.setAttribute("style", "width:50%; height:50px");
    searchDiv.setAttribute("class", "border border-1 border-success d-flex justify-content-center align-items-center");
    header.appendChild(searchDiv);

    let inputdiv = document.createElement("input");
    inputdiv.setAttribute("style", "width:100%; height:40px");
    inputdiv.setAttribute("type", "text");
    inputdiv.setAttribute("placeholder", "search items by name")
    inputdiv.setAttribute("class", "bg-white text-center rounded");
    searchDiv.appendChild(inputdiv);
    inputdiv.addEventListener('keydown', () => {
        cartContainer();
    })
